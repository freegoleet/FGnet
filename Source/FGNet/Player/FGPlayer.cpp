...
void AFGPlayer::Tick(float DeltaTime) {
	...
	if (IsLocallyControlled()) {
		...
		Remote_Transform(TargetTransform, DeltaTime);
	}
}
...
{
	if (!IsLocallyControlled()) {
		TargetTransform = TransformToSend;
	}
}

void AFGPlayer::Remote_Transform(FTransform InTransform, float DeltaTime)
{
	FRotator NewRot = FMath::RInterpTo(GetActorRotation(), InTransform.Rotator(), DeltaTime, InterpSpeed);
	FVector NewLoc = FMath::VInterpTo(GetActorLocation(), InTransform.GetLocation(), DeltaTime, InterpSpeed);

	InterpTransform.SetComponents(NewRot.Quaternion(), NewLoc, GetActorScale());
